    ...
    private func submitRegistrationPassword() async {
        ...
        do {
            ...
            let overrallResult = try await self.networkController.executeServiceAction(
                connectId: connectId,
                serviceAction: .opaqueSignInInit,
                plainBuffer: try initRequest.serializedData(),
                flowType: .single,
                onSuccessCallback: { payload in
                    do {
                        let initResponse = try Utilities.parseFromBytes(Ecliptix_Proto_Membership_OpaqueSignInInitResponse.self, data: payload)
                        ...
                        if finalizationResult.isErr {
                            let failure = EcliptixProtocolFailure.generic("Failed to process server response: \(try finalizationResult.unwrapErr().message)")
                        ...
                        return try await self.networkController.executeServiceAction(
                            connectId: connectId,
                            serviceAction: .opaqueSignInComplete,
                            plainBuffer: try finalizeRequest.serializedData(),
                            flowType: .single,
                            onSuccessCallback: { payload2 in
                                do {
                                    let finalizeResponse = try Utilities.parseFromBytes(Ecliptix_Proto_Membership_OpaqueSignInFinalizeResponse.self, data: payload2)
                                    ...
                                    if finalizeResponse.result == .invalidCredentials {
                                        ...
                                        return .failure(.generic("Invalid credentials."))
                                    }
                                    ...
                                    if verificationResult.isErr {
                                        let failure = EcliptixProtocolFailure.generic("Server authentication failed: \(try verificationResult.unwrapErr().message)")
                                        self.errorMessage = failure.message
                                        return .failure(failure)
                                    }
                                    ...
                                    return .failure(.generic("Unexpected error"))
                                }
                            })
                    } catch {
