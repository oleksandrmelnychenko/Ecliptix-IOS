...
final class SodiumInterop {
    ...
    private static func wipeLargeBuffer(_ buffer: inout Data) -> Result<Unit, SodiumFailure> {
        return Result<Unit, SodiumFailure>.Try {
            ...
            guard let baseAddress = buffer.withUnsafeMutableBytes({ $0.baseAddress }) else {
                throw SodiumFailure.memoryPinningFailed(
                    SodiumFailureMessages.getPinnedAddressFailed,
                    inner: nil
                )
            }
            ...
            return .memoryPinningFailed(String(format: SodiumFailureMessages.secureWipeFailed, buffer.count), inner: error)
        }
    }
