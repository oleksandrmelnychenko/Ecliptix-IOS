...
enum AppServiceConfigurator {
    @MainActor static func registerCoreServices() {
        ...
        let storage = KeychainStorageProvider(ttlDays: 90)
        ServiceLocator.shared.register(SecureStorageProviderProtocol.self, service: storage)
        
        AppSettingsService.shared = AppSettingsService(storage: storage)
